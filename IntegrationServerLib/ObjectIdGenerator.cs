...
{
  /// <summary>
  /// Детерминированная генерация стабильного object_id из (input, version).
  /// Нужна и northbound (IntegroController), и приёмному gRPC (IntegroGrpcImp) — общий хелпер.
  /// </summary>
  public static class ObjectIdGenerator
  {
    public static string GenerateObjectId(string input, string version)
    {
      input ??= string.Empty;
